...
namespace LunraGames
{
	public static class Deltas
	{
		...
		{
			changed = (changed && changedStaysTrue) || !EqualityComparer<T>.Default.Equals(startValue, resultValue);
			return resultValue;
		}
		...
		{
			if (objects.Length % 2 != 0) throw new ArgumentOutOfRangeException(nameof(objects), "An even number of objects must be passed for camparison");
